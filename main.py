...
async def search_anime_start(message: types.Message):
    await SearchStates.waiting_for_anime_name.set()
    await message.answer("🔍 Qidirilayotgan animening nomini kiriting:")
...
async def search_anime_process(message: types.Message, state: FSMContext):
    ...
    all_animes = await get_all_codes()
    ...
    if not results:
        await message.answer(f"❌ '{query}' bo'yicha hech qanday anime topilmadi.")
        await state.finish()
        return

    # Natijalarni inline tugmalar sifatida chiqarish
    ...
    for anime in results:
        ...
        keyboard.insert(InlineKeyboardButton(
            text=f"{title} ({code})",
            callback_data=f"search_result:{code}"
        ))
    ...
    await state.finish()
@dp.callback_query_handler(lambda c: c.data.startswith("search_result:"))
async def handle_search_result(callback: types.CallbackQuery):
    ...
    await callback.answer("✅ Kino yuklandi!")
...
from database import (
    ...
    get_all_user_ids,
    update_anime_code,
    search_anime_by_name  # ✅ Yangi funksiya qo'shildi
)
...
# Qidiruv (masalan, anime nomi bo'yicha) ✅ YANGI HOLAT
class SearchStates(StatesGroup):
    waiting_for_anime_name = State()
...
async def start_handler(message: types.Message):
    ...
    if message.from_user.id in ADMINS:
        ...
        kb.add(
            KeyboardButton("🎞 Barcha animelar"),
            KeyboardButton("🔍 Anime qidirish")  # ✅ YANGI TUGMA
        )
        kb.add(KeyboardButton("✉️ Admin bilan bog'lanish"))
        await message.answer("🎬 Botga xush kelibsiz!\nKod kiriting yoki anime qidiring:", reply_markup=kb)
...
# === 🔍 Anime qidirish ✅ YANGI FUNKSIYA
@dp.message_handler(lambda m: m.text == "🔍 Anime qidirish")
async def start_anime_search(message: types.Message):
    await SearchStates.waiting_for_anime_name.set()
    await message.answer("🔍 Qidirmoqchi bo'lgan anime nomini yozing:\n\n💡 Masalan: Naruto, One Piece, Attack on Titan")

@dp.message_handler(state=SearchStates.waiting_for_anime_name)
async def search_anime_handler(message: types.Message, state: FSMContext):
    await state.finish()
    ...
    results = await search_anime_by_name(search_query)
    
    if not results:
        await message.answer(f"❌ '{search_query}' bo'yicha hech narsa topilmadi.\n\n💡 Boshqa nom bilan qayta urinib ko'ring.")
        return
    
    # Natijalarni ko'rsatish
    if len(results) == 1:
        # Faqat bitta natija bo'lsa, to'g'ridan-to'g'ri yuborish
        anime = results[0]
        code = anime['code']
        ...
        if not await is_user_subscribed(message.from_user.id):
            markup = await make_subscribe_markup(code)
            await message.answer(
                f"✅ Topildi: *{anime['title']}*\n\n❗ Kino olishdan oldin quyidagi kanal(lar)ga obuna bo'ling:",
                parse_mode="Markdown",
                reply_markup=markup
            )
        else:
            await message.answer(f"✅ Topildi: *{anime['title']}*", parse_mode="Markdown")
            await send_reklama_post(message.from_user.id, code)
            await increment_stat(code, "searched")
            await increment_stat(code, "viewed")
    else:
        # Ko'p natija bo'lsa, ro'yxat ko'rsatish
        text = f"🔍 '{search_query}' bo'yicha topilganlar:\n\n"
        
        # Inline keyboard yaratish
        keyboard = InlineKeyboardMarkup(row_width=1)
        
        for anime in results[:10]:  # Maksimum 10 ta natija
            text += f"• *{anime['title']}* (`{anime['code']}`)\n"
            keyboard.add(InlineKeyboardButton(
                f"📥 {anime['title']}", 
                callback_data=f"select_anime:{anime['code']}"
            ))
...
# === Qidiruv natijasidan tanlash ✅ YANGI CALLBACK
@dp.callback_query_handler(lambda c: c.data.startswith("select_anime:"))
async def select_anime_callback(callback: types.CallbackQuery):
    code = callback.data.split(":")[1]
    user_id = callback.from_user.id
    
    # Anime ma'lumotlarini olish
    anime_data = await get_kino_by_code(code)
    if not anime_data:
    ...
    if not await is_user_subscribed(user_id):
        markup = await make_subscribe_markup(code)
        await callback.message.edit_text(
            f"✅ Tanlandi: *{anime_data['title']}*\n\n❗ Kino olishdan oldin quyidagi kanal(lar)ga obuna bo'ling:",
            parse_mode="Markdown",
            reply_markup=markup
        )
    else:
        await callback.message.edit_text(f"✅ Tanlandi: *{anime_data['title']}*", parse_mode="Markdown")
        await send_reklama_post(user_id, code)
        await increment_stat(code, "searched")
        await increment_stat(code, "viewed")
...
async def qollanma(message: types.Message):
    kb = (
        ...
        .add(InlineKeyboardButton("🔍 6. Qidirish funksiyasi", callback_data="help_search"))  # ✅ YANGI
    )
...
HELP_TEXTS = {
    ...
    "help_search": (  # ✅ YANGI YORDAM SAHIFASI
        "🔍 *Qidirish funksiyasi*\n\n"
        "• *'🔍 Anime qidirish'* tugmasini bosing\n\n"
        "• Anime nomini yozing (kamida 2 harf)\n\n"
        "• Agar bir nechta natija bo'lsa, kerakli animeya tanlang\n\n"
        "• Obuna bo'lib, anime epizodlarini oling\n\n"
        "💡 *Masalan:* Naruto, One Piece, Attack"
    )
}
